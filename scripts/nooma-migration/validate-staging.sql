...
BEGIN
    IF current_database() !~ '^devbrain_nooma_stage_[0-9]{8}(_[0-9]+)?$' THEN
        RAISE EXCEPTION
            'Refusing to validate database %; expected devbrain_nooma_stage_YYYYMMDD[_N]',
            current_database();
    END IF;
    ...
    IF bad_count <> 0 THEN
        RAISE EXCEPTION 'Found % non-Nooma developer identities', bad_count;
    END IF;

    IF (SELECT count(*) FROM devbrain.devs WHERE dev_id = 'patrickkelly') <> 1 THEN
        RAISE EXCEPTION 'Expected exactly one patrickkelly developer identity';
    END IF;
    ...
    WHERE m.fanout_source_session_id IS NOT NULL
      AND rs.id IS NULL;
    IF bad_count <> 0 THEN
...
\echo 'Nooma staging validation passed.'
SELECT
...
SELECT p.slug, count(*) AS memory_rows
FROM devbrain.memory m
